...
#include <stdio.h>
    ...
    extern MATRIXES bool matrixCompareEqualityZeroMatrix(matrix* matrix);
    extern MATRIXES bool matrixCompareEqualityUnitMatrix(matrix* matrix);
...
#define newLineAndNullPoint 2
    ...
    extern MATRIXES matrix* textFileScanMatrix(FILE* stream, uint16_t amountOfRow, uint16_t amountOfColumn, uint32_t modulus, uint64_t* maxMemAlloc);
    extern MATRIXES int convertIntoElementFromString(matrix* matrix, uint16_t row, uint16_t column, char* string);
    extern MATRIXES matrix* matrixBasicFileRead(FILE* stream, uint64_t* maxMemAlloc);
    extern MATRIXES int matrixBasicFileWrite(FILE* stream, matrix* matrix);
