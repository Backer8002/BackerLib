...
uint64_t factorial(uint32_t number) {
    uint64_t result = 1;
    for(uint64_t iterator = 1; iterator<= number;iterator++) result *= iterator;
    return result;
}

uint64_t partialFactorial(uint32_t startOfFactorial, uint32_t number) {
    uint64_t result = 1;
    if (startOfFactorial > number) return 0;
    for(uint64_t iterator = startOfFactorial; iterator<= number;iterator++) result *= iterator;
    return result;
}
