...
void* heapTop(const Heap* heap) {
    if (heap->container.amountOfIndexes == 0)
        return NULL;
    return heap->container.array;
}

static void internal_bubbleDown(Container* container, bool (*compare)(const void*, const void*)) {
    size_t currentIndex = 1;
    while (currentIndex * 2 <= container->amountOfIndexes) {
        if (currentIndex * 2 == container->amountOfIndexes) {
...
void heapPop(Heap* heap) {
    ...
    internal_bubbleDown((Container*) heap, heap->compare);
}
...
void heapSort(Container* container, bool (*compare)(const void*, const void*)) {
    ...
    for (size_t i = 1; i < container->amountOfIndexes; i++) {
        ...
        internal_bubbleDown(container, compare);
    }
}
