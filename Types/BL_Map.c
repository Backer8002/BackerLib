    ...
    if (nodeToRemove->left) {
        if (nodeToRemove->right) {
            ...
            if (inorderSuccessor == nodeToRemove->right) {
                ...
                inorderSuccessor->weight = nodeToRemove->weight-1;
                if (nodeToRemove->weight < 0) {
                    if (nodeToRemove->left->weight > 0)
                        inorderSuccessor = internal_rotate_left_right(inorderSuccessor);
                    else
