...
BL_ContainerError bl_unordered_container_set(BL_UnorderedContainer* container, size_t index, size_t sizeOfElement, const void* element) {
    ...
    if (index >= container->amountOfPages * BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT) {
        if (bl_unordered_container_reserve(container,index - container->amountOfElements + 1) != BL_ContainerOPSuccessful)
...
BL_ContainerError bl_unordered_container_reserve(BL_UnorderedContainer* container, size_t amount) {
    size_t currentMaxElements = container->amountOfPages * BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT;
    if (container->amountOfElements + amount <= currentMaxElements)
        return BL_ContainerOPSuccessful;

    size_t additonalNeededPages = (container->amountOfElements + amount - currentMaxElements + BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT - 1) / BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT;
    void*  newPageArr           = realloc(container->pages, sizeof *container->pages * (container->amountOfPages + additonalNeededPages));
    if (!newPageArr)
        return BL_ContainerAllocFailure;
    
    container->pages = newPageArr;

    size_t currentBitsetLen = (container->amountOfPages * BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT + 8 * sizeof *container->bitset - 1) / (8 * sizeof *container->bitset);
    size_t newBitsetLen     = ((container->amountOfPages + additonalNeededPages) * BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT + 8 * sizeof *container->bitset) / (8 * sizeof *container->bitset);
    void* newBitset = realloc(container->bitset,sizeof*container->bitset *newBitsetLen);
    if(!newBitset)
        return BL_ContainerAllocFailure;
    ...
    for (size_t allocationIterator = 0; allocationIterator < additonalNeededPages; ++allocationIterator) {
        void* page = malloc((size_t) container->byteSizeOfElement * BL_UNORDERED_CONTAINER_PAGE_SIZE_IN_AMOUNT);
        if (!page) {
            for (size_t i = 0; i < allocationIterator; ++i)
                free(container->pages[container->amountOfPages + i]);
            return BL_ContainerAllocFailure;
        }
        container->pages[container->amountOfPages + allocationIterator] = page;
    }
...
bool bl_unordered_container_is_empty(const BL_UnorderedContainer* container) {
    return container->amountOfElements == 0;
}
